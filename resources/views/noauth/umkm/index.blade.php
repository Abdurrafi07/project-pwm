...
<div class="container">
    ...
    <!-- <div class="mb-3">
        ...
        <a href="{{ route('noauth.umkm.index') }}" 
           class="btn btn-sm {{ request('kategori_id') ? 'btn-outline-secondary' : 'btn-primary' }}">
            Semua
        </a>
        @foreach ($kategoris as $kategori)
            <a href="{{ route('noauth.umkm.index', array_merge(request()->query(), ['kategori_id' => $kategori->id])) }}"
    ...
    <div class="mb-3">
        ...
        <a href="{{ route('noauth.umkm.index', ['kategori_id' => request('kategori_id')]) }}" 
           class="btn btn-sm {{ request('daerah_id') ? 'btn-outline-secondary' : 'btn-success' }}">
            Semua
        </a>
        @foreach ($daerahs as $daerah)
            <a href="{{ route('noauth.umkm.index', array_merge(request()->query(), ['daerah_id' => $daerah->id])) }}"
    ...
    <!-- <div class="mb-3">
        ...
        @foreach ($sektors as $sektor)
            <a href="{{ route('noauth.umkm.index', array_merge(request()->query(), ['sektor_id' => $sektor->id])) }}"
